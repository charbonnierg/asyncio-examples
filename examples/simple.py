...
if __name__ == "__main__":
    with Timer() as timer:
        print(f"{timer():.5f} ms -- Timer initialized")

        with AsyncioThread() as thread:

            print(f"{timer():.5f} ms -- Thread ready")
            ...
            print(f"{timer():.5f} ms -- All tasks started")
            # Let's wait for this task first
            thread.wait_threadsafe([task]).result()
            print(f"{timer():.5f} ms -- First task ended")
            future1.result()
            future2.result()
            print(f"{timer():.5f} ms -- Second and third tasks ended")
            done_task = future3.result()
            print(f"{timer():.5f} ms -- Last task ended with result: {done_task}")
